...
class AsyncStreamReaderMixin:
    ...
    def iter_chunked(self, n: int, ignore_excep: bool = False) -> AsyncStreamIterator[bytes]:
        """Returns an asynchronous iterator that yields chunks of size n.

        Python-3.5 available for Python 3.5+ only
        """
        return AsyncStreamIterator(
            lambda: self.read(n, ignore_excep)  # type: ignore[attr-defined,no-any-return]
        )
...
class StreamReader(AsyncStreamReaderMixin):
    ...
    async def read(self, n: int = -1, ignore_excep: bool = False) -> bytes:
        if(ignore_excep is True):
